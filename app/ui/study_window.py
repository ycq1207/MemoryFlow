...
class StudyWindow(QWidget):
    ...
    def _show_segment_read(self):
        ...
        hint_texts = {
            "poem": "请阅读以下诗句及释义，理解后点击下方按钮开始背�?,
            "question": "请阅读以下问题和答案，理解后点击下方按钮开始背�?,
            "vocabulary": "请阅读以下单词和释义，理解后点击下方按钮开始背�?,
            "article": "请阅读以下段落，理解后点击下方按钮开始背�?,
            "formula": "请阅读以下公式和说明，理解后点击下方按钮开始背�?,
        }
        hint = QLabel(hint_texts.get(self.card_type, "请阅读以下内容，理解后点击下方按钮开始背�?))
    ...
    def _show_segment_test(self):
        ...
        if self.is_verification:
            self.phase_label.setText(f"✍️ 默写验证 - 无提�?({self.current_segment + 1}/{total_segs})")
        else:
        ...
        prompt_texts = {
            ...
            "article": f"请输入第 {global_line_num} 段内�?,
            "formula": f"请输入第 {global_line_num} 个公�?,
        }
        line_num = QLabel(prompt_texts.get(self.card_type, f"请输入第 {global_line_num} �?))
    ...
    def _show_final_test(self):
        ...
        if self.is_verification:
            self.phase_label.setText(f"✍️ 默写验证 - 无提�?({done + 1}/{total})")
        else:
