...
class StudyWindow(QWidget):
    ...
    def _show_segment_test(self):
        ...
        line_num = QLabel(prompt_texts.get(self.card_type, f"请输入第 {global_line_num} 句"))
        ...
        for i in range(done + 1):
            ...
            if i < done:
                ...
                input_field.setPlaceholderText(placeholder_texts.get(self.card_type, "请输入内容。"))
    ...
    def _finish_poem(self):
        ...
        if self.current_index < len(self.cards):
            ...
            QMessageBox.information(
                self, "完成",
                f"今日学习已完成！\n{SHORT_REVIEW_MINUTES} 分钟后会安排一次短复习，请记得回来 ~"
            )
            self._emit_closed()
            self.close()
